...
num_runs = 3
c4_levels = [15, 30, 45, 60, 75, 90, 105]
...
for c4 in c4_levels:
    ...
    plt.plot(t_vec_valid, lfp_valid, color='red', linewidth=2, label='Average LFP')
    for i in range(num_runs):
        plt.plot(t_vec_valid, lfp_all_valid[i], alpha=0.3, label=f'Trial {i+1}')
    plt.xlabel('Time (ms)')
    plt.ylabel('Average LFP (mV)')
    plt.title(f'Average LFP Across 5 Runs for C4 = {c4} mg/dL')
    plt.legend()
    plt.tight_layout()
    plt.show()
    plt.savefig(f"average_lfp_c4{c4}.jpeg")
    ...
    plt.title(f'LFP Power Spectrum (Fourier Analysis) for C4 = {c4} mg/dL')
    plt.tight_layout()
    plt.show()
    plt.savefig(f"fourier_transform_{c4}.jpeg")
    ...
    for i in range(num_runs):
        f_spec, t_spec, Sxx = spectrogram(lfp_valid, fs=1000/dt, nperseg=256, noverlap=128)
        plt.subplot(3, 2, i+1)
        plt.pcolormesh(t_spec + 50, f_spec, Sxx, shading='gouraud')
        plt.ylim(0, 100)
        plt.xlabel('Time (ms)')
        plt.ylabel('Frequency (Hz)')
        plt.title(f'Spectrogram Trial {i+1} (C4={c4})')
    plt.suptitle(f'Spectrograms (C4 = {c4} mg/dL)', fontsize=16, y=1.02)
    plt.tight_layout()
    plt.show()
    plt.savefig(f"spectrogram_{c4}.jpeg")
